...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      url: http + '/api/Query/outwin',
      success: function (res) {
        console.log(res.data)
        if (res.data.code == 1) {
            if(res.data.code==1){
                  that.setData({
                    txt_html:res.data.data.content
                  })
              setTimeout(function () {
                that.setData({
                  show_box: true
    ...
    wx.request({
      url: http + '/api/Query/gcode',
      success: function (res) {
        console.log(res.data)
        if (res.data.code == 1) {
          that.setData({
            gzh: http + res.data.data
          })
        }
      }
    })
    ...
    if (options.record_id) {
      ...
      post_data = {
        num: wx.getStorageSync('list_'),
        openid: wx.getStorageSync('user_obj').wx_openid
      };
      url_ = '/api/Query/index';
  ...
  onShareAppMessage: function() {
    ...
    return {
      title: 'test',
      path: '/pages/result/result?record_id=' + that.data.record_id,
    }
  }
})