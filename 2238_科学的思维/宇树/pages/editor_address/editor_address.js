...
Page({
  ...
  del_:function(){
    ...
    var that=this;
    wx.request({
      url: http + '/api/user/del_address', //仅为示例，并非真实的接口地址
      method: "get",
      data:{
          id: that.data.ad_id
      },
  ...
  submit_: function () {
    ...
    wx.request({
      url: http + '/api/user/edit_address', //仅为示例，并非真实的接口地址
      data: {
        name: that.data.name,
          mobile: that.data.phone,
          address: that.data.xq,
          zip: that.data.code,
          default: that.data.df,
          id: that.data.ad_id,
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: http + '/api/user/address_info', //仅为示例，并非真实的接口地址    
      data:{
          id: options.id,
      },
      header: {
      ...
      success: function (res) {
        ...
        if (res.data.code==1){
          that.setData({
            ...
            code: res.data.data.zip,
            region: res.data.data.province ,
            xq: res.data.data.address,
       
            df: res.data.data.default
          })
